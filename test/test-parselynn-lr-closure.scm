...
;; (WIKIPEDIA REFERENCE)
;;
;; Starting from the production rules of a language,
;; at first the item sets for this language have to be determined.
;; In plain words, an item set is the list of production rules,
;; which the currently processed symbol might be part of.
;; An item set has a one-to-one correspondence to a parser state,
;; while the items within the set, together with the next symbol,
;; are used to decide which state transitions and parser action are to be applied.
;; Each item contains a marker, to note at which point
;; the currently processed symbol appears in the rule the item represents.
;; For LR(1) parsers, each item is specific to a lookahead terminal,
;; thus the lookahead terminal has also been noted inside each item.
;;
;; For example, assume a language consisting of the terminal symbols 'n', '+', '(', ')',
;; the nonterminals 'E', 'T', the starting rule 'S' and the following production rules:
;;
;; S → E
;; E → T
;; E → ( E )
...
;; T → T + n
;;
;; Items sets will be generated by analog to the procedure for LR(0) parsers.
;; The item set 0 which represents the initial state will be created from the starting rule:
;;
;; [S → • E, $]
;;
