...
(define (list-annihilate pred constant lst)
  (letrec ((process
            (lambda (lst flags)
              (cond ((null? lst) '())
                    ((car flags) (cons constant (process (cdr lst) (cdr flags))))
                    ((exists (lambda (flag) (and (not flag) (pred (car lst) (car lst))))
                              (cdr lst)
                              (cdr flags)) 
                     (cons constant (process (cdr lst) (update-flags (cdr lst) (cdr flags) (car lst) pred))))
                    (else (cons (car lst) (process (cdr lst) (cdr flags)))))))
           (exists
            (lambda (pred lst flags)
              (cond ((null? lst) #f)
                    ((car flags) (exists pred (cdr lst) (cdr flags)))
                    ((pred (car lst)) #t)
                    (else (exists pred (cdr lst) (cdr flags))))))
           (update-flags
            (lambda (lst flags el pred)
              (if (null? lst)
                  '()
                  (cons (or (car flags) (pred el (car lst)))
                        (update-flags (cdr lst) (cdr flags) el pred))))))
    (process lst (map (lambda (_) #f) lst))))
