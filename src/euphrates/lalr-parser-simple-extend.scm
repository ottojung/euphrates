...
(define (lalr-parser/simple:extend
         original additional)
  ...
  (define (parser->options parser)
    (define arguments
      (lalr-parser/simple-struct:arguments parser))

    (arguments->options arguments))

  (define (get-options x)
    (cond
     ((lalr-parser/simple-struct? x)
      (parser->options x))
     ((list? x)
      (arguments->options x))
     (else
      (raisu* :from "lalr-parser/simple:extend"
              :type 'bad-parser-type
              :message "Expected either a lalr-parser/simple-struct or arguments that defined it"
              :args (list x)))))
  ...
  (if (and (list? original)
           (list? additional))
      arguments-new
      (lalr-parser/simple arguments-new)))
