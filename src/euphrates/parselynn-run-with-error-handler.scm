...
(define (parselynn-run/with-error-handler
  ...
  (define maybefun (parselynn-struct:maybefun parser-struct))
  (if maybefun
      (maybefun token-iterator errorp)
      (let ()
        (define code (parselynn-struct:code parser-struct))
        (define actions (parselynn-struct:actions parser-struct))
